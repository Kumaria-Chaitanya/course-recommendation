...
@app.route('/admin/courses/<student_id>/<course_id>', methods=['PUT'])
@cross_origin()
def add_student_progress(student_id,course_id):
    updated_data = request.get_json()

    if not updated_data:
        return jsonify({"error": "Invalid request data"}), 400

    updated_course_info, status_code = AdminServices.add_student_progress(student_id, course_id, updated_data)
    return jsonify(updated_course_info), status_code
